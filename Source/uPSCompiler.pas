...
begin
  if FProcs = nil then
  begin
    raise EPSCompilerException.Create('This function can only be called from within the OnUses event');
  end;
...
begin
  ...
  try
    if Parser.CurrTokenId = CSTII_Function then
      ...
      Raise EPSCompilerException.Create('Unable to register function '+Name);
    Parser.Next;
    if Parser.CurrTokenId <> CSTI_Identifier then
      Raise EPSCompilerException.Create('Unable to register function '+Name);
...
{$IFNDEF PS_NOIDISPATCH}
    ...
    begin
      ...
      begin
        ...
        begin
          s := FParser.GetToken;
          if TPSInterfaceType(FType).Intf.Func_Find(s, Idx) then
          begin
            FParser.Next;
...
var
  ...
  val: PIfRVariant;
begin
  if FProcs = nil then raise EPSCompilerException.Create('This function can only be called from within the OnUses event');

  FType := GetTypeCopyLink(FType);
  if FType = nil then
    Raise EPSCompilerException.Create('Unable to register constant '+name);
...
begin
  if FProcs = nil then raise EPSCompilerException.Create('This function can only be called from within the OnUses event');
  if FType = nil then raise EPSCompilerException.Create('Invalid type for variable '+Name);
  p := TPSVar.Create;
  p.OrgName := Name;
  p.Name := Fastuppercase(Name);
  p.FType := AT2UT(FType);
...
begin
  ...
  if Typeno = nil then raise EPSCompilerException.Create('Invalid Type');
...
begin
  ...
  Parser.Free;
  if result = nil then Raise EPSCompilerException.Create('Unable to register type '+name);
end;
...
begin
  ...
  try
    if not ParseMethod(Self, '', Decl, DOrgName, pDecl, FT) then
      Raise EPSCompilerException.Create('Unable to register function '+Decl);
...
begin
  ...
  raise EPSCompilerException.Create('Unknown interface: '+ Name);
end;
{$ENDIF}
...
begin
  ...
  begin
    case FValue.FType.BaseType of
      ...
      raise EPSCompilerException.Create('Constant Value Type Mismatch');
    end;
  end else
    raise EPSCompilerException.Create('Constant Value is not assigned')
end;
...
begin
  ...
  try
    ...
    begin
      Result := False;
      {$IFDEF DEBUG} raise EPSCompilerException.Create('Unable to register '+Decl); {$ENDIF}
      exit;
    end;
    ...
    begin
      ...
      begin
        if p.Decl.ParamCount = 0 then
          Raise EPSCompilerException.Create('Not an array property');
        FDefaultProperty := I;
        exit;
      end else Raise EPSCompilerException.Create('Not a property');
    end;
  end;
  raise EPSCompilerException.Create('Unknown Property');
end;
