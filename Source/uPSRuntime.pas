...
implementation
uses
  TypInfo {$IFDEF DELPHI3UP} , ComObj {$ENDIF};
...
begin
  ...
  begin
    ...
    case pp^.PropType^.Kind of
      ...
      tkEnumeration: begin Result := GetEnumName(pp^.PropType{$IFDEF DELPHI3UP}^{$ENDIF}, GetOrdProp(Instance, pp)); exit; end;
...
begin
  ...
  if Result = '' then
    Result := Format('OLE error %.8x', [ErrorCode]);
end;
...
begin
  case TVarData(Src).VType of
    ...
    raise Exception.Create('Variant does not reference an OLE object');
  end;
    ...
    case Cmd of
      0: begin { >= }
          case var1Type.BaseType of
            ...
            btS32:
              begin
                ...
                case var2type.BaseType of
                  ...
                  btDouble: b := PSGetReal(Var1, var1type) >= tbts32(var2^);
                  btSingle: B := psGetReal(Var1, var1Type) >= tbts32(var2^);
            ...
            btCurrency: b := tbtcurrency(var1^) >= PSGetReal(Var2, var2type);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          New({$IFDEF DELPHI4UP}POleVariant{$ELSE}PVariant{$ENDIF}(DispParam.rgvarg[i].pvarVal));
          {$IFDEF DELPHI4UP}POleVariant{$ELSE}PVariant{$ENDIF}(DispParam.rgvarg[i].pvarVal)^ := Par[High(Par)-i];
        end;
      end;
      ...
      begin
        ...
        begin
          if {$IFDEF DELPHI4UP}POleVariant{$ELSE}PVariant{$ENDIF}(DispParam.rgvarg[i].pvarVal) <> nil then
            Dispose({$IFDEF DELPHI4UP}POleVariant{$ELSE}PVariant{$ENDIF}(DispParam.rgvarg[i].pvarVal));
        end;
      end;
